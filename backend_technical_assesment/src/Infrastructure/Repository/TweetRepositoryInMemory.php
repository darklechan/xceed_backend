...
use App\Domain\Repository\TweetRepository;

class TweetRepositoryInMemory implements TweetRepository
{
    ...
    {
        $randomEntries = array_slice($this->tweets, -$limit, $limit);
        $tweets = [];

        foreach ($randomEntries as $randomEntry) {
            $tweets[] = new Tweet($randomEntry);
        }
